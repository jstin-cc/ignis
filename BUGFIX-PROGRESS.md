...
- [x] **#5 TUI macht bei jedem Tick Full-Scan, ignoriert Position-Tracking (ADR-011)**
...
- [x] **#8 WalkDir-Errors werden komplett geschluckt**
...
- [x] **#11 Race zwischen `file_identity()` und `File::open()`**
...
- [x] **#16 Sidechain-Events fließen ungefiltert in alle Summen**
...
- [x] **#12 Token-Generator ist nicht crypto-zufällig**
...
- [x] **#13 `ALLOWED_ORIGINS` matcht weder Vite-Default noch alle Tauri-Origins**
...
- [x] **#15 RwLock-Poisoning wird stillschweigend ignoriert**
...
- [x] **#14 `config.json`-Fehlermeldung lügt auf Non-Windows**
...
- [x] **#17 Polling im Tray ohne `AbortController`**
...
- [x] **#18 Notification feuert sofort beim Öffnen, wenn Block ≥80 % ist**
...
- [x] **#20 `winusage export` schreibt nur nach stdout**
  - Backlog (steht in `NEXT.md`): `--output <file>`-Flag.

- [x] **#21 `home_projects_dir()` Fehlerklasse meldet falsches `var`**
  - Datei: `src/config.rs:82-90` — analog #14.

- [x] **#22 `SessionDto.is_active` mit `active_id == Some(...)`**
  - Funktioniert, ist aber unnötig fragil. Defensive Refaktorierung.

- [x] **#23 `BlockPanel.computeBurnRate` mischt Client- und Server-Uhr**
  - Datei: `tray/src/components/BlockPanel.tsx:74-81` — Drift → negativ.

- [x] **#24 `Snapshot.sessions` wächst unbegrenzt**
...
2026-04-20 · #5+#8+#11 · scanner: scan_incremental, WalkDir-Errors, file_identity race · 3dd0cfc
2026-04-20 · #12+#14+#21 · config: getrandom-Token, korrekte Fehlermeldungen · 76e0d17
